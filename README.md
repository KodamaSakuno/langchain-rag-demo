...
> |---|---|---|
> | 直接命中 | 13/16 → 15/16 | 70% → 74% |
> | 跨文档综合 | 6/8 → 7/8 | 64% → 82% |
> | API 时效陷阱 | 6/8 → 8/8 | 61% → 86% |
> | 超纲拒答 | 4/4 → 4/4 | — |
>
> - **多 Agent 全面领先，且越难的题型差距越大**：来源命中 80.56%→94.44%（+13.9pp），答案要点召回 66.09%→78.96%（+12.9pp）；时效陷阱题命中 6/8→8/8、要点召回 61%→86%。代价是延迟 18.0s→74.5s、token ×2.0
> - **运行间波动需如实看待**：单 Agent 组两次全量运行命中率为 88.89%/80.56%（DeepSeek 非确定性 + 难题敏感），多 Agent 组为 91.67%/94.44%——多 Agent 的检索-审校循环同时提升了稳定性。表中取第二次运行（要点断言修正后的同口径数据）
...
> - **诚实度**：4 道超纲题两组全部明确拒答（含 Milvus 集群部署 K8s、用 LangChain 训练图像分类模型两道纯域外题）。文档题存在主 Agent 检索不足就提前拒答的波动（抽查确认与审校无关——审校均判"通过"）
> - **审校员覆盖代码级错误**：除论断核对外还检查代码示例的 import 完整性与参数有效性。存疑 10/36，抽查确认真阳性——如某题草稿写出不存在的 `from langchain_chat_models import ChatAnthropic`（正确为 `langchain_anthropic`）被抓出并修正；同时能区分占位代码与真实错误，不误伤
> - **规划员按需启用**：多 Agent 组 28/36 题走了 `plan_queries` 拆题，复杂题才付出规划成本
> - **审校员误报可量化调优**：题库 v1 上初版 prompt 误把措辞不精确判"存疑"（4/36），明确"只有实质性错误才判存疑"后降至 2/36 且指标无退化
